// TODO: Use document.getElementById to select the element and log properties.
// Tasks guide: create a variable `element` and log id, classList, innerText, innerHTML, attributes.
// Example starter:
// const element = document.getElementById('sample');
// console.log(element.id);
//1. Capture the specific DOM element by its ID and store it in a variable
const element = document.getElementById("sample");

// 2. Use console.log to display element.id, element.classList, element.innerText, element.innerHTML, and element.attributes
console.log("Element ID:", element.id);
console.log("Element ClassList:", element.classList);
console.log("Element InnerText:", element.innerText);
console.log("Element InnerHTML:", element.innerHTML);
console.log("Element Attributes:", element.attributes);

// 3. Use the debugger to inspect element
	// debugger statement removed
